...
from rank_analysis_for_google_search.settings import PROXIES
...
try:
    ...
    try:
        ...
        with open(user_agents_file) as fp:  # type: ignore
            user_agents_list = [_.strip() for _ in fp.readlines()]
except Exception:
...
def get_page(url, user_agent=None, verify_ssl=True):
    ...
    if PROXIES:
        request_ = s.get(
            url, headers={"User-Agent": user_agent}, verify=verify_ssl, proxies=proxies
        )
    else:
        request_ = s.get(url, headers={"User-Agent": user_agent}, verify=verify_ssl)
    html = request_.text
    try:
        cookie_jar.save()
    except Exception:
